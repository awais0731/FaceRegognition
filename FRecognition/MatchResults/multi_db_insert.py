...
class MultiDBInsert:
    ...
    def insert_FrMatch(request_obj, data, MatchedFromDb, isCro, MatchedFromDbName, MatchingThreshold):
        ...
        for i in range(5):
            if(i < len(data['result']['paths'])):
                ...
                picture = MultiDBInsert.GetFileNameOrCroNoFromPath(str(data['result']['paths'][i]),MatchedFromDb, isCro, MatchedFromDbName)
                ...
                # if isCro:
                #     parameters["CroNo"] = MultiDBInsert.encryption_cro_no(picture)
                # else:
